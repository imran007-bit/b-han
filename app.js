// ═══ i18n: EN / বাংলা / 한국어 ═══
window.LANG=(function(){try{return localStorage.getItem('bhan_lang')||'en'}catch(e){return 'en'}})();
window.T={
en:{home:'Home',shop:'Shop',bag:'Bag',orders:'Orders',login:'Login',begin:'Begin the ritual',signin:'Sign in',register:'Create account',myorders:'My orders',checkout:'Checkout',placeorder:'Send order on WhatsApp',subtotal:'Subtotal',shipping:'Shipping',total:'Total',free:'FREE',bagempty:'Your bag is empty',bagemptysub:'Beautiful skin starts with one product',startshop:'Start shopping',delivery:'Delivery details',payment:'Payment method',track:'Track your order',trackbtn:'Track',fullname:'Full name',phone:'Phone number',address:'Full address',email:'Email',password:'Password',coupon:'Coupon code',apply:'Apply',addtobag:'Add to bag',welcomeback:'Welcome back',forgotpw:'Forgot password?',logout:'Log out',profile:'Profile',save:'Save',search:'Search products…',all:'All',bestsellers:'Best sellers',newarrivals:'New arrivals',viewall:'View all',also:'You may also like',inside:'Inside Dhaka',outside:'Outside Dhaka',skintype:'What is your skin type?',next:'NEXT',results:'Your matches',orderid:'Order ID'},
bn:{home:'হোম',shop:'শপ',bag:'ব্যাগ',orders:'অর্ডার',login:'লগইন',begin:'শুরু করুন',signin:'সাইন ইন',register:'অ্যাকাউন্ট খুলুন',myorders:'আমার অর্ডার',checkout:'চেকআউট',placeorder:'হোয়াটসঅ্যাপে অর্ডার পাঠান',subtotal:'সাবটোটাল',shipping:'ডেলিভারি চার্জ',total:'মোট',free:'ফ্রি',bagempty:'আপনার ব্যাগ খালি',bagemptysub:'সুন্দর ত্বক শুরু হয় একটি প্রোডাক্ট দিয়ে',startshop:'কেনাকাটা শুরু করুন',delivery:'ডেলিভারি তথ্য',payment:'পেমেন্ট পদ্ধতি',track:'অর্ডার ট্র্যাক করুন',trackbtn:'ট্র্যাক',fullname:'পুরো নাম',phone:'ফোন নম্বর',address:'সম্পূর্ণ ঠিকানা',email:'ইমেইল',password:'পাসওয়ার্ড',coupon:'কুপন কোড',apply:'প্রয়োগ',addtobag:'ব্যাগে যোগ করুন',welcomeback:'আবার স্বাগতম',forgotpw:'পাসওয়ার্ড ভুলে গেছেন?',logout:'লগ আউট',profile:'প্রোফাইল',save:'সেভ',search:'প্রোডাক্ট খুঁজুন…',all:'সব',bestsellers:'বেস্ট সেলার',newarrivals:'নতুন এসেছে',viewall:'সব দেখুন',also:'আপনার পছন্দ হতে পারে',inside:'ঢাকার ভিতরে',outside:'ঢাকার বাইরে',skintype:'আপনার ত্বকের ধরন কী?',next:'পরবর্তী',results:'আপনার ম্যাচ',orderid:'অর্ডার আইডি'},
ko:{home:'홈',shop:'쇼핑',bag:'장바구니',orders:'주문',login:'로그인',begin:'시작하기',signin:'로그인',register:'회원가입',myorders:'내 주문',checkout:'결제하기',placeorder:'왓츠앱으로 주문하기',subtotal:'소계',shipping:'배송비',total:'합계',free:'무료',bagempty:'장바구니가 비어 있어요',bagemptysub:'아름다운 피부는 하나의 제품에서 시작됩니다',startshop:'쇼핑 시작',delivery:'배송 정보',payment:'결제 방법',track:'주문 조회',trackbtn:'조회',fullname:'이름',phone:'전화번호',address:'주소',email:'이메일',password:'비밀번호',coupon:'쿠폰 코드',apply:'적용',addtobag:'담기',welcomeback:'다시 오셨네요',forgotpw:'비밀번호를 잊으셨나요?',logout:'로그아웃',profile:'프로필',save:'저장',search:'제품 검색…',all:'전체',bestsellers:'베스트셀러',newarrivals:'신상품',viewall:'전체 보기',also:'함께 보면 좋은 제품',inside:'다카 시내',outside:'다카 외곽',skintype:'피부 타입이 무엇인가요?',next:'다음',results:'맞춤 제품',orderid:'주문 번호'}
};
function tr(k){var L=window.T[window.LANG]||window.T.en;return L[k]||window.T.en[k]||k}
...
const B = {
  ...
  _renderAuth(){
    ...
    if(m==='login'){
      h=`<div class="auth-card">
        ...
        <input class="auth-input" id="auth-pw" type="password" placeholder="${tr('password')}" onkeydown="if(event.key==='Enter')B.doLogin()"/>
        <button class="auth-btn" onclick="B.doLogin()">${tr('signin')} →</button>
        <div class="auth-switch"><a onclick="B.switchAuth('forgot')">${tr('forgotpw')}</a></div>
      ...
      h=`<div class="auth-card">
        ...
        ${e?`<div class="auth-err">${esc(e)}</div>`:''}
        <input class="auth-input" id="auth-name" placeholder="${tr('fullname')}"/>
        ...
        <button class="auth-btn" onclick="B.doRegister()">${tr('register')} →</button>
  ...
  _renderQuiz(){
    ...
    $('quiz-body').innerHTML=`
      ...
      <div class="quiz-q"><h4>${esc(step.qk?tr(step.qk):step.q)}</h4><div class="q-opts">${optsH}</div></div>
  ...
  // Track
  cycleLang(){
    var order=['en','bn','ko'];var i=(order.indexOf(window.LANG)+1)%3;
    window.LANG=order[i];try{localStorage.setItem('bhan_lang',window.LANG)}catch(e){}
    render();
  },
  goPDSlide(i){const g=document.getElementById('pd-gallery');if(!g)return;g.scrollTo({left:i*g.clientWidth,behavior:'smooth'});},
  _pdScroll(g){clearTimeout(window._pdT);window._pdT=setTimeout(()=>{const i=Math.round(g.scrollLeft/Math.max(1,g.clientWidth));document.querySelectorAll('.pd-dot').forEach((d,x)=>d.classList.toggle('on',x===i));},80);},
  zoomImg(url){
    if(!url)return;
  ...
  _trackHTML(){
    ...
    let h=`<div class="track-input-row"><input id="track-input-id" class="track-input" placeholder="BHN1234567" value="${order?esc(order.id):''}" onkeydown="if(event.key==='Enter')B.submitTrack()"/><button class="track-btn" onclick="B.submitTrack()">${tr('trackbtn')}</button></div>`;
...
function buildWAMsg(order,settings){
  ...
  const area=order.customer.area==='outside'?tr('outside'):tr('inside');
...
function render(){
  ...
  html+=`<header class="header" role="banner">${backBtn}<div class="logo" onclick="B.go('home')" role="button" aria-label="Go to home page"><span class="logo-text">b·han</span><span class="logo-sub">${esc(s.tagline||'Seoul → Dhaka')}</span></div><div class="head-icons"><button class="icon-btn" onclick="B.cycleLang()" title="Language" aria-label="Change language" style="font-size:10px;font-weight:800">${window.LANG==='bn'?'বাং':window.LANG==='ko'?'한국':'EN'}</button>${userBtn}<button class="icon-btn" onclick="B.go('cart')" title="Cart" aria-label="Shopping cart with ${cc} items">◯${cc>0?`<span class="badge">${cc}</span>`:''}</button></div></header>`;
  ...
  const ordersNav=u?nv('orders','≡',tr('orders')):`<button class="nav-item" onclick="B.openAuth('login')"><span class="ni-icon">→⊕</span><span class="ni-label">${tr('login')}</span></button>`;
  root.innerHTML=html;
  ...
  navbar.innerHTML=`<nav class="nav" role="navigation" aria-label="Main navigation">${nv('home','◐',tr('home'))}${nv('products','✦',tr('shop'))}${nv('cart','◯',tr('bag'))}${ordersNav}</nav>`;
...
function renderPD(){
  ...
  const img=imgs[0]||'';
  const imgH=imgs.length>1
    ?`<div class="pd-gallery" id="pd-gallery" onscroll="B._pdScroll(this)">${imgs.map(im=>`<div class="pd-slide"><img src="${esc(im)}" alt="${esc(p.name)}" onclick="B.zoomImg('${esc(im)}')" loading="lazy"/></div>`).join('')}</div>`
    :(img?`<img src="${esc(img)}" alt="${esc(p.name)}" onclick="B.zoomImg('${esc(img)}')" style="width:100%;height:100%;object-fit:contain;padding:20px;position:relative;z-index:1;cursor:zoom-in"/>`:'<span class="pd-hero-icon">✶</span>');
  ...
  return `<div class="page on">
    ...
    ${imgs.length>1?`<div class="pd-dots">${imgs.map((im,i)=>`<span class="pd-dot${i===0?' on':''}" onclick="B.goPDSlide(${i})"></span>`).join('')}</div>`:''}
    <div class="pd-info">
...
// ─── CART ───
function renderCart(){
  if(!State.cart.length)return `<div class="page on"><div class="cart-empty"><div class="ce-icon">◯</div><div class="ce-msg">${tr('bagempty')}</div><button class="btn-ink" style="margin-top:20px" onclick="B.go('products')">Begin the ritual →</button></div></div>`;
  ...
  return `<div class="page on"><div class="sec-hd"><div><div class="sec-sub">${State.cart.length} items</div><div class="sec-title">Your bag</div></div></div>${shipBar}<div class="cart-list">${items}</div><div class="coupon-row"><input class="coupon-input" id="coupon-input" placeholder="Coupon code" value="${esc(State.coupon?State.coupon.code:'')}"/><button class="coupon-btn" onclick="B.applyCoupon(document.getElementById('coupon-input').value)">${tr('apply')}</button></div><div class="cart-total"><div class="ct-row"><span>${tr('subtotal')}</span><span>${fmt(t.subtotal)}</span></div><div class="ct-row"><span>${tr('shipping')}</span><span>${t.shipping?fmt(t.shipping):'FREE'}</span></div>${t.discount?`<div class="ct-row" style="color:#5a7a3a"><span>Discount</span><span>-${fmt(t.discount)}</span></div>`:''}<div class="ct-row grand"><span>${tr('total')}</span><span>${fmt(t.total)}</span></div></div><button class="checkout-btn" onclick="B.go('checkout')">Continue to checkout →</button></div>`;
}
...
function renderCheckout(){
  ...
  return `<div class="page on"><div class="pd-back"><button class="pd-back-btn" onclick="B.go('cart')">←</button><div class="pd-label">Checkout</div><div style="width:38px"></div></div>${verifyReminder}<div class="co-form"><div class="co-card"><div class="co-title">${tr('delivery')}</div><label class="label">Full Name</label><input class="input" placeholder="Your name" value="${esc(c.name)}" oninput="B.setCustomer('name',this.value)"/><label class="label">Phone</label><input class="input" type="tel" placeholder="01XXXXXXXXX" value="${esc(c.phone)}" oninput="B.setCustomer('phone',this.value)"/><label class="label">Address</label><textarea class="input textarea" placeholder="House, Road, Area, City" oninput="B.setCustomer('address',this.value)">${esc(c.address)}</textarea><label class="label">Delivery Area</label><div class="row2"><button class="pay-opt ${c.area==='inside'?'on':''}" onclick="B.setCustomer('area','inside')"><div class="pay-name">${tr('inside')}</div><div style="font-size:10px;color:var(--muted);margin-top:3px">${fmt(s.shippingInsideDhaka||80)}</div></button><button class="pay-opt ${c.area==='outside'?'on':''}" onclick="B.setCustomer('area','outside')"><div class="pay-name">${tr('outside')}</div><div style="font-size:10px;color:var(--muted);margin-top:3px">${fmt(s.shippingOutsideDhaka||130)}</div></button></div></div><div class="co-card"><div class="co-title">${tr('payment')}</div><div class="pay-grid">${payOpt('bkash','🌸','bKash')}${payOpt('nagad','🟠','Nagad')}${payOpt('cod','💵','Cash on Delivery')}${s.rocketNumber?payOpt('rocket','🚀','Rocket'):''}</div>${payInfo}</div><div class="co-card"><div class="co-title">Order summary</div><div class="ct-row"><span>${State.cart.length} items</span><span>${fmt(t.subtotal)}</span></div><div class="ct-row"><span>${tr('shipping')}</span><span>${t.shipping?fmt(t.shipping):'FREE'}</span></div>${t.discount?`<div class="ct-row" style="color:#5a7a3a"><span>Discount</span><span>-${fmt(t.discount)}</span></div>`:''}<div class="ct-row grand"><span>${tr('total')}</span><span>${fmt(t.total)}</span></div></div></div>${waH}</div>`;
}

// ─── INIT ───
// ─── MY ORDERS ───
function renderMyOrders(){
  if(!State.user) return `<div class="page on"><div class="cart-empty"><div class="ce-icon">→⊕</div><div class="ce-msg">Sign in to see your orders</div><button class="btn-ink" style="margin-top:20px" onclick="B.openAuth('login')">${tr('signin')} →</button></div></div>`;
  // Load orders async
...
function renderProfile(){
  if(!State.user) return `<div class="page on"><div class="cart-empty"><div class="ce-icon">→⊕</div><div class="ce-msg">Sign in to see your profile</div><button class="btn-ink" style="margin-top:20px" onclick="B.openAuth('login')">${tr('signin')} →</button></div></div>`;
  const u=State.user;
  ...
  return `<div class="page on">
    ...
    <div class="profile-card">
      ...
      <div class="row2">
        <button class="pay-opt ${(u.area||State.customer.area)==='inside'?'on':''}" onclick="State.customer.area='inside';if(State.user)State.user.area='inside';render()"><div class="pay-name">${tr('inside')}</div></button>
        <button class="pay-opt ${(u.area||State.customer.area)==='outside'?'on':''}" onclick="State.customer.area='outside';if(State.user)State.user.area='outside';render()"><div class="pay-name">${tr('outside')}</div></button>
      </div>
