...
const B = {
  ...
  closeAuth(){document.getElementById('auth-modal').classList.remove('on')},
  ...
  // Quiz
  openQuiz(){this._closeAllModals();State._qs=0;State._qa={};$('quiz-modal').classList.add('on');this._renderQuiz()},
  closeQuiz(){$('quiz-modal').classList.remove('on')},
  _renderQuiz(){
  ...
  // Track
  openTrack(){this._closeAllModals();State.trackOrder=null;State.trackError='';$('track-modal').classList.add('on');$('track-body').innerHTML=this._trackHTML();setTimeout(()=>{const i=document.getElementById('track-input-id');if(i)i.focus()},100)},
  closeTrack(){$('track-modal').classList.remove('on')},
  async submitTrack(){
  ...
  _trackHTML(){
    ...
    if(!order)return h+`<div class="track-msg info">Enter your Order ID (starts with BHN)</div>`;
  ...
  async sendOrder(which){
    ...
    const t=computeTotals(),orderId='BHN'+Array.from(crypto.getRandomValues(new Uint8Array(4))).map(b=>b.toString(10)).join('').slice(0,7);
    const order={
      id:orderId,
...
// ═══════════════════════════════════════════
function render(){
  const root=$('app');if(State.loading){root.innerHTML='<div class="loading">Preparing your ritual...</div>';return}
  ...
  html+=`<nav class="nav" role="navigation" aria-label="Main navigation">${nv('home','◐','Home')}${nv('products','✦','Shop')}${nv('cart','◯','Bag')}${ordersNav}</nav>`;
  root.innerHTML=html;
  const sI=document.getElementById('search-input');if(sI)sI.addEventListener('input',e=>B.setSearch(e.target.value));
}
...
function renderPD(){
  ...
  const img=p.image||(p.images&&p.images[0])||'';
  const imgH=img?`<img src="${esc(img)}" alt="${esc(p.name)}" style="width:100%;height:100%;object-fit:contain;padding:20px;position:relative;z-index:1"/>`:'<span class="pd-hero-icon">✶</span>';
  ...
  return `<div class="page on">
    ...
    <div class="pd-dots"><span class="pd-dot on"></span><span class="pd-dot"></span><span class="pd-dot"></span></div>
    <div class="pd-info">
...
function renderCheckout(){
  ...
  // Must be logged in AND verified to checkout
  if(!State.user){
  ...
  if(!State.user.emailVerified){
    return `<div class="page on"><div class="cart-empty"><div class="ce-icon">✉️</div><div class="ce-msg">Verify your email first</div><div style="margin-top:12px;font-size:12px;color:var(--muted);max-width:260px;margin-left:auto;margin-right:auto;line-height:1.5">Check <strong>${esc(State.user.email||'')}</strong> for a verification link, then come back.</div><div style="margin-top:16px;display:flex;gap:8px;justify-content:center"><button class="btn-ink" onclick="B.resendVerify()">Resend email</button><button class="btn-outline" onclick="B.refreshVerify()">I've verified</button></div></div></div>`;
  }
