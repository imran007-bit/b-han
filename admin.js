...
let _adminAttempts = [];
async function login(emailOrPw, pw){
  ...
  // ── SECURE MODE: Firebase email/password + admins/{uid} allowlist ──
  if (useFirebase && typeof auth !== 'undefined' && auth && pw !== undefined) {
    try {
      const cred = await auth.signInWithEmailAndPassword((emailOrPw||'').trim().toLowerCase(), pw);
      const uid = cred.user.uid;
      // Must be listed in the 'admins' collection (created once via Firebase Console)
      let isAdmin = false;
      try {
      ...
      if (!isAdmin) {
        ...
        A.loginErr = 'This account is not an admin.';
        renderLogin();
        return;
      }
      A.loggedIn = true;
      A.loginErr = '';
      try { sessionStorage.setItem('bhan_admin', JSON.stringify({ ts: Date.now(), uid })); } catch(e){}
      boot();
      return;
    } catch(e) {
      A.loginErr = (e && (e.code==='auth/wrong-password'||e.code==='auth/user-not-found'||e.code==='auth/invalid-credential'))
        ? 'Invalid email or password.' : ('Login failed: ' + (e.message||''));
      renderLogin();
      return;
    }
  }

  // ── OFFLINE FALLBACK: local hashed password (only when Firebase unavailable) ──
  const ok = await verifyAdminPassword(pw !== undefined ? pw : emailOrPw);
  if (ok) {
    A.loggedIn = true;
    A.loginErr = '';
    try { sessionStorage.setItem('bhan_admin', JSON.stringify({ ts: Date.now() })); } catch(e){}
    boot();
  } else {
...
function logout(){
  ...
  if (useFirebase && typeof auth !== 'undefined' && auth) { try { auth.signOut(); } catch(e){} }
  renderLogin();
}

function renderLogin(){
  const fbMode = (useFirebase && typeof auth !== 'undefined' && auth);
  $a('app').innerHTML = `
    <div class="login-pg">
      <div class="login-card">
        <div class="login-logo">B</div>
        <div class="login-title">B HAN Admin</div>
        <div class="login-sub">${fbMode ? 'Sign in with your admin account' : 'Enter password to continue'}</div>
        ${A.loginErr ? `<div class="login-err">${escA(A.loginErr)}</div>` : ''}
        ${fbMode ? `
          <input id="em-input" class="login-input" type="email" placeholder="Admin email" onkeydown="if(event.key==='Enter')document.getElementById('pw-input').focus()"/>
          <input id="pw-input" class="login-input" type="password" placeholder="Password" onkeydown="if(event.key==='Enter')login(document.getElementById('em-input').value,this.value)"/>
          <button class="login-btn" onclick="login(document.getElementById('em-input').value,document.getElementById('pw-input').value)">Sign In</button>
        ` : `
...
function renderProductForm(){
  ...
  return `
    ...
    <div class="form-card">
      ...
      <label class="label">Product Images (up to 3)</label>
      <div class="img-uploader">
        <div style="display:flex;gap:10px;flex-wrap:wrap">
          ${[0,1,2].map(i=>{
            const imgs=(p.images&&p.images.length?p.images:(p.image?[p.image]:[]));
            const cur=imgs[i]||'';
            return `<div class="img-slot" style="flex:1;min-width:90px">
              <div class="img-preview" id="p-img-preview-${i}" style="height:90px">
                ${cur?`<img src="${escA(cur)}" alt="preview" style="width:100%;height:100%;object-fit:cover;border-radius:10px"/>`:'<span class="img-placeholder">📷 '+(i+1)+'</span>'}
              </div>
              <label class="img-btn upload-btn" style="margin-top:6px;font-size:11px;padding:6px">
                <input type="file" accept="image/*" style="display:none" onchange="uploadProductImage(event,${i})"/>
                📤 ${cur?'Change':'Add'}
              </label>
              <input type="hidden" id="p-img-hidden-${i}" value="${escA(cur)}"/>
              <div id="p-img-progress-${i}" style="display:none;font-size:10px;color:#666;text-align:center;margin-top:3px"></div>
            </div>`;
          }).join('')}
        </div>
        <div style="margin-top:10px">
          <button class="img-btn url-btn" type="button" onclick="toggleImgUrl()" style="font-size:12px">🔗 Paste URL instead (slot 1)</button>
          <div id="p-img-url-row" style="display:none;margin-top:8px">
            <input class="input" id="p-img" value="${escA((p.images&&p.images[0])||p.image||'')}" placeholder="https://... or paste URL" oninput="previewImgUrl(this.value)"/>
          </div>
        </div>
      </div>
...
async function saveProduct(){
  ...
  // Images: collect from 3 slots + optional URL field
  const slotImgs=[];
  for(let i=0;i<3;i++){
    const h=$a('p-img-hidden-'+i);
    if(h&&h.value.trim()){const u=safeImgUrl(h.value.trim());if(u)slotImgs.push(u);}
  }
  const urlImg=$a('p-img');
  if(urlImg&&urlImg.value.trim()){const u=safeImgUrl(urlImg.value.trim());if(u&&!slotImgs.includes(u))slotImgs.unshift(u);}
  p.images=slotImgs;
  p.image=slotImgs[0]||'';  // keep .image for backwards compat
...
// Security: only allow safe image sources (https or data:image base64)
...
function previewImgUrl(url){
  const preview=$a('p-img-preview-0');
  const hidden=$a('p-img-hidden-0');
  if(!preview)return;
  url=safeImgUrl(url);if(url){
...
async function uploadProductImage(event,slot){
  slot=(slot===undefined?0:slot);
  ...
  const preview=$a('p-img-preview-'+slot);
  const progress=$a('p-img-progress-'+slot);
  const hidden=$a('p-img-hidden-'+slot);
...
function start(){
  ...
  if (useFirebase && typeof auth !== 'undefined' && auth) {
    // Secure mode: wait for Firebase auth state, then verify admin allowlist
    auth.onAuthStateChanged(async (user) => {
      if (user && sessionOk) {
        try {
          const doc = await db.collection('admins').doc(user.uid).get();
          if (doc.exists) { A.loggedIn = true; boot(); return; }
        } catch(e){}
      }
      A.loggedIn = false;
      renderLogin();
    ...
    if (sessionOk) { A.loggedIn = true; boot(); } else renderLogin();
  }
}
